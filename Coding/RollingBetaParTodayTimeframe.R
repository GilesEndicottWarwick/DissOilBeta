...
#THE MINIMUM TIMESTEPS TO USE FOR BETA CALC IE 24 = 2Y BETA
betaCalcLength = ( 12 * 1 );
...
collectRegressionData = foreach(iSymbol = 1:dim(allSymbols)[1], .combine=rbind , .packages = parPackages ) %dopar% 
{
  ...
  {
      #SUBSET THE DATA
      thisSubset = combData1[iTime:dim(combData1)[1],];
...
#ADD NUMBER OF MONTHS OF DATA TO DATASET
#collectRegressionData = collectRegressionData %>%
 #                       mutate(nMonths = )

#WRITE AN OUTPUT FILE
save(collectRegressionData,file="C:/Users/giles/OneDrive/MBA/Dissertation/Data/RollingBetaTimeframe.Rdata")
