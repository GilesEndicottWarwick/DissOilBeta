...
collectRegressionData = foreach(iSymbol = 1:dim(allSymbols)[1], .combine=rbind , .packages = parPackages ) %dopar% 
{
  ...
  thisSymbol = allReturnsData %>%
                ...
                ungroup() %>%
                dplyr::select(SymbolDesc,YearMonth,SymbolRet);
  ...
  thisMarket = allReturnsData %>%
                ...
                ungroup() %>%
                dplyr::select(YearMonth,MarketRet);
  ...
  thisOil = allReturnsData %>%
            ...
            ungroup() %>%
            dplyr::select(YearMonth,OilRet);
  
  
  
  #COMBINE THEM
  combData1 = thisSymbol %>%
              inner_join(thisMarket,by=c("YearMonth")) %>%
              inner_join(thisOil,by=c("YearMonth")) %>%
  ...
  testReg1A = lm(data = combData1, MarketRet  ~  OilRet);
  summary(testReg1A)
  
  #TEST ROBUST REGRESSION
  testReg2A = lmrob(data = combData1,  MarketRet  ~  OilRet )
  summary(testReg2A)
  
  #TEST QUANTILE REGRESSION
  testReg3A = rq(data = combData1, MarketRet  ~  OilRet )
  summary(testReg3A)
  ...
  for( iTime in 1:(dim(combData1)[1]-betaCalcLength+1))
  {
      ...
      #DO THIS REGRESSION 
      subsetReg1A = lm(data = thisSubset, MarketRet ~ OilRet);
      ...
      #ROBUST REGRESSION 
      subsetReg2A = lmrob(data = thisSubset,  MarketRet ~ OilRet );
      ...
      #QUANTILE REGRESSION
      subsetReg3A = rq(data = thisSubset, MarketRet ~ OilRet );
...
save(collectRegressionData,file="C:/Users/giles/OneDrive/MBA/Dissertation/Data/RollingBetaOilvsSP500.Rdata")
