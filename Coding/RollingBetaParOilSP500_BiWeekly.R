...
ReturnsDataWithOil = (IndustryReturnsDataWithOil_OUT_BiWeekly);
...
collectRegressionData = foreach(iSymbol = 1:dim(allSymbols)[1], .combine=rbind , .packages = parPackages ) %dopar% 
{
  ...
  thisSymbol = allReturnsData %>%
                ...
                ungroup() %>%
                dplyr::select(SymbolDesc,Year,YearBiWeek,SymbolRet,MarketRet,OilRet);
...
save(collectRegressionData,file="C:/Users/giles/OneDrive/MBA/Dissertation/Data/RollingBetaIndustryBiWeekly.Rdata")
