...
ReturnsDataWithOil = (IndustryReturnsDataWithOil_OUT_Weekly);
...
collectRegressionData = foreach(iSymbol = 1:dim(allSymbols)[1], .combine=rbind , .packages = parPackages ) %dopar% 
{
  ...
  thisSymbol = allReturnsData %>%
                ...
                ungroup() %>%
                dplyr::select(SymbolDesc,Year,YearWeek,SymbolRet,MarketRet,OilRet);
...
save(collectRegressionData,file="C:/Users/giles/OneDrive/MBA/Dissertation/Data/RollingBetaIndustryWeekly.Rdata")
