...
#HOW MANY TIEMSTEPS TO USE FOR BETA CALC IE 24 = 2Y BETA, 52 weeks = 1 year, 26 = 1 year of bi weekly entries
betaCalcLength = ( 26 * 1 );
...
collectRegressionData = foreach(iSymbol = 1:dim(allSymbols)[1], .combine=rbind , .packages = parPackages ) %dopar% 
{
  ...
  for( iTime in 1:(dim(combData1)[1]-betaCalcLength+1))
  {
      ...
      thisOutputRow = data.frame(
                                ...
                                Index              = iTime,
                                start              = min(thisSubset$YearBiWeek),
                                end                = max(thisSubset$YearBiWeek),
...
collectRegressionData = collectRegressionData %>%
                        left_join(DateSeq1,by=c("start"="YearBiWeek"))

#WRITE AN OUTPUT FILE
save(collectRegressionData,file="C:/Users/giles/OneDrive/MBA/Dissertation/Data/RollingBetaBiWeekly.Rdata")
