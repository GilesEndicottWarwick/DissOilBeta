...
#GET START AND END OF WEEK ONLY
allReturns_S1 = Skeleton_1 %>%
  filter(!is.na(SymbolValue)) %>%
  mutate(Year = year(Date)) %>%
  mutate(Week = week(Date)) %>%
  group_by(Symbol,SymbolDesc,Industry,SubIndustry,Year,Week) %>%
...
allReturns_S2 = allReturns_S1 %>%
  ...
  group_by(Industry,Year,Week,Date) %>%
  summarise(
...
RiskFreeData =   Skeleton_1 %>%
  ...
  mutate(Year = year(Date)) %>%
  mutate(Week = week(Date)) %>%
  group_by(Symbol,SymbolDesc,Year,Week) %>%
...
RiskFreeDataJoin = RiskFreeData %>%
  dplyr::select(Year,Week,RiskFree = SymbolValue);


#SUBTRACT RISK FREE RATE FROM RETURNS 
IndustryReturnsWithRiskFree = allReturns_S2 %>%
  left_join(RiskFreeDataJoin,by=c("Year","Week")) %>%
...
IndustryReturnsDataWithOil = IndustryReturnsWithRiskFree %>%
  left_join(OilData, by=c("Year","Week")) %>%
  left_join(SP500Data, by=c("Year","Week")) %>%
...
IndustryReturnsDataWithOil_OUT_Weekly = IndustryReturnsDataWithOil %>%
...
save(IndustryReturnsDataWithOil_OUT_Weekly,file="C:/Users/giles/OneDrive/MBA/Dissertation/Data/MonthlyIndustryReturnsWeekly.Rdata");
